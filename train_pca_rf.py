...
pca_rf_pipeline = Pipeline([
    ...
    ('rf', RandomForestClassifier(n_estimators=100, max_depth=None))  # Optimized Random Forest parameters
])
...
for seed in seeds:
    ...
    y_pred = pca_rf_pipeline.predict(X_test)
...
plt.title('1000 Seeds in a Pipelined PCA + Random Forest Model\nAverage Accuracy: {:.2f}'.format(average_accuracies))
