...
UTC_INTERVAL_PRIOR = 30
UTC_INTERVAL_POST = 90
N_EVENTS = 15
...
def average_sentiments(dataframes):
    ...
    average_values = df_concat[feature_columns].mean().to_dict()
...
def separate_by_affiliation(
    ...
    for chunk in pd.read_csv(file_path, chunksize=10000, usecols=columns_to_keep):
        # Convert timestamps to numeric, handling errors
        chunk['created_utc'] = pd.to_numeric(chunk['created_utc'], errors='coerce')
        # Convert string representations of lists to actual lists
        chunk['labels'] = chunk['labels'].apply(lambda x: ast.literal_eval(x) if isinstance(x, str) else x)

        # Early exit if chunk start is beyond the end_utc
        if chunk['created_utc'].iloc[0] > end_utc:
            break

        # Continue filtering if chunk end is before start_utc
        if chunk['created_utc'].iloc[-1] < start_utc:
            continue
...
def bin_by_drive(filename):
    ...
    for item in drive_data:
        intervals.append((item["start_utc"], item["end_utc"], item["delta_win_prob"]))
